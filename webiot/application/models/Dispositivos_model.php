...
class Dispositivos_model extends CI_Model {
   ...
   public function getDispositivosProyecto($proyecto){
     $this->db->where("device_estado","1");
     $this->db->where("device_proyecto",$proyecto);
     $resultado = $this->db->get("devices");
     return $resultado->result();
   }

   public function getDispositivosAplicacion($aplicacion){
     $this->db->where("device_estado","1");
     $this->db->where("device_aplicacion",$aplicacion);
     $resultado = $this->db->get("devices");
     return $resultado->result();
   }
